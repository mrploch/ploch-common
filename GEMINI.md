...
- Treat ContextStream-first behavior as mandatory policy: run `context(...)` first, then `search(mode="auto", ...)`
  before local discovery.
- Keep `mcp_config.json` valid and minimal: preserve non-ContextStream servers and only update the `contextstream`
  block.
