...
removes these `master` fallbacks must land on `main` afterwards. Historical `change-log/` entries
that mention `master` are deliberately left alone — they record what was true at the time.
