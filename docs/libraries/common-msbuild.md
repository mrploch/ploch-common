...
- Builds from `master`, `main`, or a tag matching `v\d+\.\d+\.\d+` are treated as public releases; all other branches produce pre-release packages. Both branch names are accepted during the `master` → `main` transition (#282).
