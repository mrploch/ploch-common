...
{
    public class RepositoryAsync<TDataContext, T, TId> : IRepositoryAsync<T, TId> where TDataContext : DbContext where T : AuditableEntity<TId>
    {
        private readonly TDataContext _dbContext;

        public RepositoryAsync(TDataContext dbContext)
        {
