...
public abstract class JsonAsyncSerializerWithSettingsTests<TSerializer, TSerializerSettings> : JsonSerializerWithSettingsTests<TSerializer, TSerializerSettings> where TSerializer : IAsyncSerializer<TSerializerSettings>
{
    [Theory]
    [AutoMockData]
    ...
    {
        await TestDeserializeAsync((sut, stream, cancellationToken) => sut.DeserializeAsync<TestRecords.TestType4>(stream, SettingsConfigurationAction, cancellationToken));
    }
    ...
    public async Task DeserializeAsync_NonGeneric_with_settings_should_correctly_deserialize_data()
    {
        await TestDeserializeAsync(async (sut, stream, cancellationToken) =>
                                   {
                                       var deserializedObject = await sut.DeserializeAsync(stream, typeof(TestRecords.TestType4), SettingsConfigurationAction, cancellationToken);
